...
# 1. 현재 파일(yolo_detector.py)의 절대 경로를 가져옵니다.
current_file_path = os.path.abspath(__file__)

# 2. infrastructure 폴더 -> src 폴더 -> backend 폴더 순으로 거슬러 올라갑니다.
# os.path.dirname을 세 번 써서 'backend' 폴더 위치를 찾습니다.
project_root = os.path.dirname(os.path.dirname(os.path.dirname(current_file_path)))

# 3. 'backend' 폴더 아래에 있는 static/model/best.pt 경로를 완성합니다.
MODEL_PATH = os.path.join(project_root, 'static', 'model', 'best.pt')

# [확인용] 터미널에 경로가 어떻게 찍히는지 보세요.
print(f"--- 모델 경로 체크: {MODEL_PATH}")
